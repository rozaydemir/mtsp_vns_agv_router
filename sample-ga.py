...
def initialize_population():
    ...
    for i in range(population_size):  # Popülasyon boyutu
        ...
        while available_pickups:
            ...
            for v_id in vehicles.keys():
                temp_route, temp_cost = greedy_insert(solution, v_id, pickup_node, delivery_node)
...
def sort_random_request():
    global pickupNodes

    items = list(pickupNodes.items())
    random.shuffle(items)

    # Tekrar sözlük haline getirme
    shuffled_data = {i + 1: value for i, (_, value) in enumerate(items)}

    pickupNodes = shuffled_data
...
data = "Instances/vestel-fabrika-verisi.txt"
